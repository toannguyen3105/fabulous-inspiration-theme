...
Hệ thống tài liệu được lưu trữ ngay trong mã nguồn để dễ dàng quản lý phiên bản
cùng với code.

### Workflow: Sync Docs to Wiki

- **File kích hoạt**: `.github/workflows/deploy-wiki.yml`
- **Thời điểm chạy**: Khi có code mới được push vào nhánh `main` và có thay đổi
  trong thư mục `docs/`.
- **Hành động**:
...
- **v1.0 (2026-01-13)**: Thiết lập quy trình CI/CD ban đầu (Theme Check & Wiki
  Sync).
- **v1.1 (2026-01-13)**: Cập nhật Wiki Workflow (chuyển sang
  `spenserblack/actions-wiki`) để sửa lỗi path.
- **v1.2 (2026-01-13)**: Sửa lỗi tham số `wiki-directory` cho workflow deploy
  wiki.
- **v1.3 (2026-01-13)**: Cấp quyền `permissions: contents: write` cho workflow
  để khắc phục lỗi Authentication (Error 128).
- **v1.4 (2026-01-14)**: Cập nhậtWiki Workflow (chuyển sang
  `spenserblack/actions-wiki`) để sửa lỗi path.
